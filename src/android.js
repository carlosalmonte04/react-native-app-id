    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            if (bundleId) {
                ...
                changes[gradleBuild] = markup.replace(
                    /applicationId .*?\".*?\"/,
                    `applicationId "${newID}"`,
                );
            } else {
            ...
            if (err.code === "ENOENT") {
                reject(
                    "Could not read android build gradle, are you sure you are in a react native project?",
                );
            }
        }
        ...
        if (newName) {
            try {
                ...
                if (matchName) {
                    console.log(
                        `ANDROID: Found android display name ${matchName[2]}`,
                    );
                    changes[stringsXml] = markup.replace(
                        searchDisplayName,
                        "$1" + newName + "$3",
                    );
                } else {
                ...
                if (err.code === "ENOENT") {
                    reject(
                        "Could not read android strings.xml, are you sure you are in a react native project?",
                    );
                }
            }
        }
        console.log("ANDROID: Successfully changed to desired values");
        resolve(changes);
    });
};
