...
module.exports = (newID, newName) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            reject(
                `${srcpath} directory not found, are you sure you are in a react native project?`,
            );
        }
        ...
        const folder =
            folders &&
            folders
                .find(f => f.indexOf(".xcodeproj") > -1)
                .replace(".xcodeproj", "");
        const plist = folders && path.resolve(dir, `ios/${folder}/Info.plist`);
        const pbxproj =
            folders &&
            path.resolve(dir, `ios/${folder}.xcodeproj/project.pbxproj`);
        let changes = {};
        ...
        fs.readFile(plist, "utf8", (err, markup) => {
            ...
            const searchBundleId = new RegExp(
                "(<key>CFBundleIdentifier</key>\n.*?<string>)(.*?)(</string>)",
                "m",
            );
            const searchDisplayName = new RegExp(
            ...
            if (matchId) {
                if (matchId[2] === "$(PRODUCT_BUNDLE_IDENTIFIER)") {
                    reject(
                        "Please move the Bundle ID into the Info.plist for this to work.",
                    );
                }

                console.log(`IOS: Found iOS bundle ID ${matchId[2]}`);
                changes[plist] = markup.replace(
                    searchBundleId,
                    "$1" + newID + "$3",
                );
            } else {
            ...
            if (newName) {
                if (matchName) {
                    ...
                    changes[plist] = changes[plist].replace(
                        searchDisplayName,
                        "$1" + newName + "$3",
                    );
                } else {
            ...
            console.log("IOS: Successfully changed to desired values");
            resolve(changes);
        });
    });
};
